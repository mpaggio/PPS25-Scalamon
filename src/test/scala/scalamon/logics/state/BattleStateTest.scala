...
class BattleStateTest extends AnyWordSpec with Matchers with StateFixtures:
  "A BattleState" should:
    ...
    "initialize with correct players and pokemons" in:
      battle.user.team("Pikachu").currentHp shouldEqual 39
      battle.enemy.team("Bulbasaur").currentHp shouldEqual 44
    ...
    "correctly update active enemy pokemon" in:
      ...
      newState.enemy.team("Bulbasaur").currentHp shouldEqual 34

    "correctly update user bench pokemon" in:
      type Move = BattleState => BattleState
      val masochistMove: Move = _ user (_ bench (_ damage 5))
      val newState = masochistMove(battle)
      newState.user.team("Pikachu").currentHp shouldEqual 39
      newState.user.team("Charmander").currentHp shouldEqual 40

    "apply move conditionally to user team" in:
      type Move = BattleState => BattleState
      val healAllMove: Move = _ user (_.allThat(ps => ps.currentHp < 40)(_ heal 10))
      // Pikachu is 39 so it gets healed, Charmander is 45 so it is skipped.
      val newState = healAllMove(battle)
      newState.user.team("Pikachu").currentHp shouldEqual 49
      newState.user.team("Charmander").currentHp shouldEqual 45
    ...
    "modify stats to enemy active pokemon" in:
      ...
      newState.enemy.team("Bulbasaur").modifiedStats.attack.toInt shouldEqual 43
    ...
    "chain multiple moves correctly" in:
      ...
      // Initial: Enemy active(Bulbasaur)=44, Atk=48. User active(Pikachu)=39, bench(Charmander)=45
      // attack: Enemy active HP -> 34
      // masochist: User bench (Charmander) HP -> 40
