...
import PowerPoints.*
import Accuracy.*
import scalamon.domain.moves.MoveCategory.*
...
class MoveTest extends org.scalatest.funsuite.AnyFunSuite:
  ...
  test("Moves should be of two possible types (damage and status)"):
    ...
    """import scalamon.domain.moves.*
      |scalamon.domain.moves.StatusMove("Thunder wave", powerPointsFromInt(32), accuracyFromPercent(90), Electric)""".stripMargin should compile
  ...
  test("Status moves should have a name, pp and accuracy"):
    val testedMove: Move = StatusMove("Thunder", powerPointsFromInt(16), accuracyFromPercent(70), Electric)
    testedMove.name shouldBe "Thunder"
  ...
  test("Move accuracy should not be created without a value of type Power, a value of type PP and a value of type Accuracy (only PP and Accuracy for status moves)"):
    ...
    assertDoesNotCompile("""StatusMove("Thunder wave", 32, 90, Electric)""")
    assertDoesNotCompile("""StatusMove("Thunder wave", powerPointsFromInt(32), 90, Electric)""")
    assertDoesNotCompile("""StatusMove("Thunder wave", 32, accuracyFromPercent(90), Electric)""")
  ...
  test("Moves should be created only with stats valid values"):
    ...
    assertThrows[IllegalArgumentException](StatusMove("Thunder wave", powerPointsFromInt(0), accuracyFromRatio(-0.5), Electric))
